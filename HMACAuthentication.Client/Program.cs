...
namespace HMACAuthentication.Client
{
    class Program
    {
        ...
        static async Task RunAsync()
        {
            ...
            string apiBaseAddress = "https://localhost:44312/";

            CustomDelegatingHandler customDelegatingHandler = new CustomDelegatingHandler();

            HttpClient client = HttpClientFactory.Create(customDelegatingHandler);

            Order order = new Order { OrderID = 10248, CustomerName = "Taiseer Joudeh", ShipperCity = "Amman", IsShipped = true };

            HttpResponseMessage response = await client.PostAsJsonAsync<Order>(apiBaseAddress + "api/Orders", order);
